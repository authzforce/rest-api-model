...
//		final String serviceBaseURL = "https://az.testbed.fi-ware.eu/authzforce";
		...
		/**
		 * Set the SSL/TLS client parameters, change the configuration in cxfClient.xml, check your
		 * keystore/truststore paths in particular.
		 */
		...
		/**
		 * Request some authorization decision from my domain's PDP First build the XACML request.
		 * First build the XACML request
		 */
		...
		{
			System.out.println("Authorization decision: " + result.getDecision() + "; status code: "
					+ result.getStatus().getStatusCode().getValue() + "; status message: " + result.getStatus().getStatusMessage() + "; "
